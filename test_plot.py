...
class SubplotAnimation(animation.TimedAnimation):
    def __init__(self, sim_id = 70,db_name = 'stigmergy.db', colormap = 'plasma'):

        self.player = SimPlayer(sim_id, db_name)
        self.player.get_steps()
        ...
        self.imshow_opts = {'cmap':plt.get_cmap(cmaps[colormap]),
                               ...
                               'origin':'bottom'}
        Z = self.player.map
        self.map = ax0.imshow(Z, interpolation='None',**self.imshow_opts)
        # ax1.set_xlim(-1, 1)
        # ax1.set_ylim(-2, 2)
