...
if __name__ == '__main__':
    qry = ["CREATE TABLE \"sim\" ( `ID` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `status` TEXT NOT NULL DEFAULT 'INITIALIZED', `recording` TEXT, `timestamp` DATETIME DEFAULT CURRENT_TIMESTAMP, `initializer` TEXT, `steps_recorded` INTEGER )"]
    qry.append("CREATE TABLE `sim_settings` ( `sim_id` INTEGER NOT NULL UNIQUE, `n_agents` integer, `dt` numeric, `steps` integer, `deploy_style` text, `deploy_timing` text, `deploy_timing_args` text, `evap_rate` numeric, FOREIGN KEY(`sim_id`) REFERENCES `sim`(`ID`) ON DELETE CASCADE, PRIMARY KEY(`sim_id`) )")
    ...
    qry.append("CREATE TABLE \"ant_settings\" ( `sim_id` INTEGER NOT NULL UNIQUE, `l` NUMERIC, `sens_offset` NUMERIC, `gain` NUMERIC, `noise_gain` NUMERIC, `sens_fun` TEXT, `deposit_fun` TEXT, FOREIGN KEY(`sim_id`) REFERENCES `sim`(`ID`) ON DELETE CASCADE, PRIMARY KEY(`sim_id`) )")
